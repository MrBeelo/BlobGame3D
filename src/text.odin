...
color := rl.WHITE, bordered := false, border_thickness := f32(5), border_color := rl.BLACK) {
	if(bordered) do for i in -1..=1 do for j in -1..=1 do if(i != 0 || j != 0) {
		rl.DrawTextEx(GetFont(font_name, font_type), to_cstr(text), pos + {f32(i) * border_thickness, f32(j) * border_thickness}, 
			font_size, font_spacing, border_color) }
...
color := rl.WHITE, shakiness := rl.Vector2{2, 1.5}, shake_length: f32 = 2, modifier := "", bordered := false, border_thickness := f32(5), border_color := rl.BLACK) {
	...
	DrawText(text, {pos.x + off_x, pos.y + off_y}, font_size, font_spacing, font_name, font_type, color, bordered, border_thickness, border_color)
}
...
color := rl.WHITE, shakiness := rl.Vector2{1, 0.75}, shake_length: f32 = 2, bordered := false, border_thickness := f32(5), border_color := rl.BLACK) {
	built_string := ""
	cursor_x := pos.x
	for char, index in text {
		...
		DrawTextShaky(str, {cursor_x, pos.y}, font_size, font_spacing, font_name, font_type, color, 
			shakiness, shake_length, string(fmt.ctprintf("%f", cursor_x)), bordered, border_thickness, border_color)
...
color := rl.WHITE, spikyness := rl.Vector2{2, 1.5}, bordered := false, border_thickness := f32(5), border_color := rl.BLACK) {
	offset_x := rand.float32_range(-spikyness.x, spikyness.x)
	offset_y := rand.float32_range(-spikyness.y, spikyness.y)
	DrawText(text, {pos.x + offset_x, pos.y + offset_y}, font_size, font_spacing, font_name, font_type, color, bordered, border_thickness, border_color)
}

DrawTextCenterX :: proc(text: string, pos_y: f32, font_size: f32, font_spacing: f32 = 5, 
font_name := FontName.CHANGA_ONE, font_type := FontType.REGULAR, color := rl.WHITE, bordered := false, 
border_thickness := f32(5), border_color := rl.BLACK) {
	text_size := MeasureText(text, font_size, font_spacing, font_name, font_type)
	DrawText(text, {SCREEN_SIZE.x / 2 - text_size.x / 2, pos_y}, font_size, font_spacing, 
		font_name, font_type, color, bordered, border_thickness, border_color)
}

DrawTextCenterXY :: proc(text: string, font_size: f32, font_spacing: f32 = 5, 
font_name := FontName.CHANGA_ONE, font_type := FontType.REGULAR, color := rl.WHITE, bordered := false, border_thickness := f32(5), border_color := rl.BLACK) {
	text_size := MeasureText(text, font_size, font_spacing, font_name, font_type)
