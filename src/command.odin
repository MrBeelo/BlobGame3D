...
ManageCommands :: proc(args: []string) {
	...
	case "points": {
		val := hlp.parse(args[2], int)
		if args[1] == "set" do run_stats.points = val
		if args[1] == "add" do run_stats.points += val
	}
	case "saferoom": if IsInMainGame() do BeginSaferoomEnterSequence()
	}
}

empty: string
CommandMenu :: proc() -> Menu { return NewMenu(
	update = proc(buttons: []Button) {
		char_pressed := rl.GetCharPressed()
		cmd_text = strings.concatenate({cmd_text, hlp.to_string(char_pressed)})
		if rl.IsKeyPressed(.BACKSPACE) do cmd_text = hlp.string_pop(cmd_text)
		if rl.IsKeyPressed(.RIGHT_SHIFT) do cmd_text = empty
	...
	draw = proc(buttons: []Button) {
		...
		DrawTextStatic(cmd_text, POS, FONT_SIZE, FONT_SPACING, .CHANGA_ONE, .REGULAR, rl.WHITE)
